...
var replaceAframe = function($) {
  _.chain($('aframe[src]')).each(function(el) {
    gutil.log(
      'Deprecation: Aframes like this will not be supported soon: <aframe src="' +
        el.getAttribute('src') +
        '">'
    );
...
var replaceCodepen = function($) {
  _.chain($('codepen[source]')).each(function(el) {
    ...
    var codepenIframe =
      "<iframe height='444' src='" + embedUrl + "' frameborder='0'></iframe>";
  ...
  if ($('codepen').length) {
    gutil.log(
      'Warning: There seems to be a rogue codepen tag:' + $('codepen').first()
    );
  }
};
...
var replaceYoutube = function($) {
  ...
  if ($('youtube').length) {
    gutil.log(
      'Warning: There seems to be a rogue youtube tag:' + $('youtube').first()
    );
  }
};
...
var setTargetAttribute_blank = function($) {
  $(
    '[href]:not([href*="courses.thinkful.com"]):not([href*="thinkful.com"])'
  ).each(function(i, el) {
    el = $(el);
    $(el).replaceWith(
